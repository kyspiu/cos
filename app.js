...
const cardScale = document.getElementById("cardScale");
const cardScaleValue = document.getElementById("cardScaleValue");
const darkMode = document.getElementById("darkMode");

const defaultSettings = { cardScale: 100, dark: false };
const cardScaleLimits = { min: 50, max: 100, step: 5 };

function applySettings(settings) {
    const requestedScale = Number(settings.cardScale);
    const scale = Number.isFinite(requestedScale)
        ? Math.min(cardScaleLimits.max, Math.max(cardScaleLimits.min, requestedScale))
        : defaultSettings.cardScale;
    const normalizedScale = Math.round(scale / cardScaleLimits.step) * cardScaleLimits.step;
    document.documentElement.style.setProperty("--card-scale", `${normalizedScale / 100}`);
    document.body.classList.toggle("dark-mode", Boolean(settings.dark));
    cardScale.value = normalizedScale;
    cardScaleValue.value = `${normalizedScale}%`;
    cardScaleValue.textContent = `${normalizedScale}%`;
    darkMode.checked = Boolean(settings.dark);
}

function saveSettings() {
    const settings = { cardScale: Number(cardScale.value), dark: darkMode.checked };
    localStorage.setItem("mowik-settings", JSON.stringify(settings));
    applySettings(settings);
}

function loadSettings() {
    try {
        applySettings({ ...defaultSettings, ...JSON.parse(localStorage.getItem("mowik-settings") || "{}") });
    } catch {
...
function resetSettings() {
    localStorage.removeItem("mowik-settings");
    applySettings(defaultSettings);
}
...
function fitCardLabel(label) {
    ...
    while ((label.scrollHeight > label.clientHeight + 1 || label.scrollWidth > label.clientWidth + 1) && fontSize > 8) {
        fontSize -= 1;
...
function createAudioControls(item) {
    ...
    recordButton.setAttribute("aria-label", `Nagraj własną wymowę: ${item.name}`);
    ...
    if (item.type === "category") {
        ...
        deleteButton.setAttribute("aria-label", `Usuń kategorię ${item.name}`);
        ...
        deleteButton.type = "button";
        deleteButton.textContent = "×";
        deleteButton.setAttribute("aria-label", `Usuń słowo ${item.name}`);
...
async function startRecording(item, recordButton) {
    ...
    try {
        ...
        recordButton.textContent = "■";
        recordButton.setAttribute("aria-label", `Zatrzymaj nagrywanie: ${item.name}`);
        ...
        recorder.addEventListener("stop", async () => {
            ...
            if (audioBlob.size > 0) await uploadItemAudio(item, audioBlob);
        }, { once: true });
        recorder.start(250);
    } catch (error) {
...
async function uploadItemAudio(item, audioBlob) {
    ...
    const filePath = `${table}/${item.id}/${Date.now()}.${extension}`;
    const { error: uploadError } = await supabaseClient.storage
        .from("word-audio")
        .upload(filePath, audioBlob, { contentType: audioBlob.type || "audio/webm", cacheControl: "0" });
    ...
    if (updateError) {
        ...
        window.alert("Plik wysłano, ale nie udało się zapisać go przy elemencie.");
        return;
    }
...
async function createCategory(event) {
    ...
    const { data: newCategory, error } = await supabaseClient
        ...
        .select("id, name, emoji, color, audio_url, parent_id, sort_order")
        .single();

    if (error) {
        console.error("Nie udało się dodać kategorii:", error);
        categoryMessage.textContent = `Nie udało się zapisać kategorii: ${error.message || "sprawdź konfigurację Supabase"}`;
        return;
    }
    ...
    state.categories.push({
        ...categoryToInsert,
        ...newCategory,
        audio_url: newCategory.audio_url || null,
        type: "category"
    });
    closeCategoryDialog();
    render();
...
function speakCurrentSentence() {
    const sentenceItems = state.sentence.filter((word) => word.audio_url);
...
cardScale.addEventListener("input", saveSettings);
